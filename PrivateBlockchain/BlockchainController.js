...
class BlockchainController {
    ...
    // Endpoint that allow Submit a Star, it is first needed to `requestOwnership` to have the message (POST endpoint)
    submitStar() {
    ...
    // This endpoint allows to ensure the blockachain is valid
    checkChainValid() {
        ...
        this.app.get("/validation", async (req, res) => { 
            ...
            try {
                if(valid.length === 0){
                    return res.status(200).send("All the Blocks are valid");
                } else {
                    return res.status(500).send(valid);
                }
            } catch (error) {
                return res.status(500).send("An error happened!");
            }
